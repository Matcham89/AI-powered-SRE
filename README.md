...
| 10 | Waits for all 26 apps to reach Synced+Healthy (up to 25 min) |
...
**App-of-Apps with automated sync.** A single root ArgoCD Application bootstraps all 26 apps from one kustomize manifest. Automated sync with self-heal means the cluster continuously reconciles to git state — no manual `argocd app sync` required after a change is pushed.
